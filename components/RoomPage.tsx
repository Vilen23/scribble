    ...
    <div className="flex justify-center  flex-col w-[100vw] px-[250px]">
      ...
      <div className="flex justify-center w-full gap-10 mt-10">
        <div className="w-[20%] border-r-2 border-t-2">{room.roomUsers.map((user)=>(
            <div className="flex justify-start items-center text- md gap-2 py-2 pl-5 border-b-2" key={user.id}>
                <Avatar>
                    <AvatarFallback className="bg-black/10 text-sm" >{user.name[0]}</AvatarFallback>
                </Avatar>
                <p className={`${user.name===session.data?.user?.name?"text-black font-medium cursor-pointer":"cursor-pointer text-black/70 font-normal hover:text-black"}`}>{user.name===session.data?.user?.name?`${user.name}(You)`:`${user.name}`}</p>
            </div>
        ))}</div>
